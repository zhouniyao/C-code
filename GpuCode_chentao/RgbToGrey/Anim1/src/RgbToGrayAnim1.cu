...
int main(void)
{
    DataBlock data;
    start_time = clock();
    CPUAnimBitmap bitmap("nature_monte.bmp", &data);
    data.bitmap = &bitmap;
    HANDLE_ERROR(cudaMalloc((void** )&data.dev_bitmap, bitmap.image_size()));
    HANDLE_ERROR(cudaMemcpy(data.dev_bitmap,bitmap.get_ptr(),bitmap.image_size(),cudaMemcpyHostToDevice));
