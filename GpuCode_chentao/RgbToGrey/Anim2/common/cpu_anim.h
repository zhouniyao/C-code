...
struct CPUAnimBitmap
{
    ...
    CPUAnimBitmap(const char *file_name, void *d = NULL)
    {
        LoadBMPFile(&pixels, &width, &height, file_name);
    ...
    {
        CPUAnimBitmap* bitmap = *(get_bitmap_ptr());
        bitmap->fAnim(bitmap->dataBlock,rand() % (bitmap->width * bitmap->height));
        glutPostRedisplay();
    }
    ...
    static void Draw(void)
    {
        ...
        //glDrawPixels(bitmap->width, bitmap->height, GL_LUMINANCE,GL_UNSIGNED_BYTE, bitmap->pixels);
        glutSwapBuffers();
    }
};
